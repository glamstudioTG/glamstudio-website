...
>(({ className, accentColor, children = "Get Started", ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn(
        "relative flex items-center text-center px-6 py-3 overflow-hidden font-medium transition-all bg-[#850e35] rounded-md group",
        className,
      )}
      {...(props as any)}
    >
      ...
      <span className="relative w-full flex items-center justify-center text-white transition-colors duration-200 ease-in-out group-hover:text-white">
        <Calendar className="mr-2 mb-0 inline-block text-white" />
        {children}
      </span>
    </button>
  );
